/*
 *  PlayState.cpp
 *  Gameplay using physics (Box 2D)
 *
 *  Created by Marcelo Cohen on 05/15.
...
void PlayState::init()
{
    ...
    bplayer = phys->newRect(PLAYER, // player id,
                                &player, // ptr to sprite
                                50, // density (with the size, defines object mass)
                                0.1, // friction coeff
                                0.1  // restitution coeff
    );
    bplayer->SetFixedRotation(true);

    bghost  = phys->newCircle(GHOST, &ghost, 50, 0.1, 0.1);

    // Add collision objects from second layer
