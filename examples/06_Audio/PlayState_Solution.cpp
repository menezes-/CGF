...
void PlayState::setup(cgf::Game* game)
{
    ...
    // Load boing sound and set params
    ...
    // Music
    // from Portal 2 soundtrack - http://www.thinkwithportals.com/music.php
    ...
    // Sheed sound
    sheepSoundBuffer.loadFromFile("data/audio/sheep.wav");
    sheepSound.setBuffer(sheepSoundBuffer);
    sheepSound.setMinDistance(50.f);
    sheepSound.setAttenuation(1.f);

    // Boom sound
    boomSoundBuffer.loadFromFile("data/audio/kaboom.wav");
    boomSound.setBuffer(boomSoundBuffer);
    boomSound.setAttenuation(0);
}
...
void PlayState::update(cgf::Game* game)
{
    ...
    // Play boing sound if ball touches the ground
    if(phys->haveContact(BALL_ID, GROUND_ID))
    {
        float vel = pball->GetLinearVelocity().y;
        if(vel > 3)
        {
            cout << "Bump!" << endl;
            if(boingSound.getStatus() == sf::Sound::Stopped) {
                boingSound.play();
            }
        }
    }
    ...
    if(sheep.isVisible()) {
        sheepSound.setPosition(sheep.getPosition().x, sheep.getPosition().y, 0);
        if(sheepSound.getStatus() == sf::Sound::Stopped && rand()%1000 > 950)
            sheepSound.play();
    }

    // Update audio listener position
    sf::Listener::setPosition(ball.getPosition().x, ball.getPosition().y, 0); // "listener"
    sf::Listener::setDirection(0,0,-1); // listener orientation (facing "forward")
